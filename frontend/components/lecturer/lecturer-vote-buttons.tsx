...
export function LecturerVoteButtons({ lecturerId, upvotes, downvotes, myVote }: LecturerVoteButtonsProps) {
    const queryClient = useQueryClient();
    const { user } = useAuth();

    const mutation = useMutation({
        mutationFn: (voteType: 'UPVOTE' | 'DOWNVOTE') => academicService.voteLecturer(lecturerId, voteType),
        onSuccess: (data) => {
    ...
    return (
        <div className="flex items-center gap-3">
            <Button
                variant="outline"
                size="sm"
                className={`flex items-center gap-2 transition-all ${isUpvoted
            ...
            <Button
                variant="outline"
                size="sm"
                className={`flex items-center gap-2 transition-all ${isDownvoted
