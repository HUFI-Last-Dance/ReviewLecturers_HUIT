    ...
    return (
      <div className="bg-slate-50 dark:bg-slate-800 p-6 rounded-xl text-center border border-slate-200 dark:border-slate-700 border-dashed transition-colors duration-300">
        <h3 className="font-bold text-slate-700 dark:text-slate-300 mb-2">
          {t('review.have_you_studied')}
        </h3>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
          {t('review.share_experience')}
        </p>
        <Button onClick={() => router.push('/login')} variant="outline">
          {t('review.login_to_write')}
        </Button>
      </div>
    );
  }
        ...
        <div>
          ...
          {errors.content ? (
            <p className="text-red-500 text-xs mt-2">
              {errors.content.message === 'MIN_LENGTH_10'
                ? t('review.validation_content_min')
        ...
        <div className="flex items-center justify-between mt-4">
          <label className="flex items-center gap-2 cursor-pointer select-none">
            ...
            <span className="text-sm text-slate-600 dark:text-slate-400">
              {t('review.post_anonymous')}
            </span>
          </label>

          <Button type="submit" isLoading={isSubmitting || mutation.isPending} className="px-6">
            {t('review.submit')}
          </Button>
        </div>
      </form>
