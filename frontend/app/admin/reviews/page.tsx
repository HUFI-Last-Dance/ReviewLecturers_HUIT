                    ...
                    <tbody className="divide-y divide-slate-100">
                        {reviews.map((review: any) => (
                            <tr key={review.id} className="hover:bg-slate-50 transition-colors group">
                                <td className="p-4">
                                    <p className="text-slate-900 line-clamp-2" title={review.content}>{review.content}</p>
                                    <div className="mt-1 text-xs text-slate-400">
                                        {new Date(review.createdAt).toLocaleDateString()} • {review.isAnonymous ? 'Ẩn danh' : 'Công khai'}
                                    </div>
                                </td>
                                <td className="p-4">
                        ...
                        {reviews.length === 0 && (
                            <tr>
                                <td colSpan={5} className="p-8 text-center text-slate-400">Không có review nào</td>
                            </tr>
                        )}
                    </tbody>
                </table>
