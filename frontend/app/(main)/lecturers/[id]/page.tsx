...
export default function LecturerDetailPage() {
  ...
  const terms = termsData?.data || EMPTY_TERMS;
  const teachingAssignments = data?.data?.teachingAssignments || EMPTY_ASSIGNMENTS;
  ...
  }, [teachingAssignments, terms]);

  // Filter and sort assignments
  const filteredAssignments = useMemo(() => {
    if (teachingAssignments.length === 0) return [];

    let assignments = [...teachingAssignments];

    // Filter by selected term
    if (selectedTermId !== 'all') {
      assignments = assignments.filter((assignment) => assignment.term.id === selectedTermId);
    }
    ...
    return assignments;
  }, [teachingAssignments, selectedTermId, terms]);
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  {t('lecturers.teaching_assignments')}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {lecturerTerms.map((term) => {
              const count = data.data.teachingAssignments.filter(
                (assignment) => assignment.term.id === term.id,
              ).length;
              return (
              ...
              <div>
                ...
                {assignment.classCode ? (
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                    {t('lecturers.class')}: {assignment.classCode}
                  </p>
                ) : null}
              </div>
