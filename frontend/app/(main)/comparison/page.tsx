...
import { Lecturer, TeachingAssignment } from '@/types/academic';
  ...
  const { data: lecturers, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return responses.map(
        (res) => res.data as Lecturer & { teachingAssignments: TeachingAssignment[] },
      );
    },
    ...
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <h2 className="text-2xl font-bold mb-2 text-slate-900 dark:text-slate-100">
          Cần chọn ít nhất 2 giảng viên
        </h2>
        <p className="text-slate-500 mb-6">
          Vui lòng quay lại danh sách và chọn giảng viên để so sánh.
        </p>
        <Link
          href="/lecturers"
          className="inline-flex items-center justify-center rounded-lg font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 disabled:pointer-events-none disabled:opacity-50 active:scale-95 bg-gradient-to-r from-blue-600 to-indigo-600 text-white hover:opacity-90 shadow-lg shadow-blue-500/30 border-0 h-10 px-4 py-2"
        >
          Quay lại danh sách
        </Link>
      </div>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 overflow-x-auto pb-8">
        {lecturers.map((lecturer) => (
          <div
            key={lecturer.id}
            className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-lg border border-slate-100 dark:border-slate-700 flex flex-col pt-12 relative overflow-hidden"
          >
            ...
            <div className="flex flex-col items-center mb-8 relative z-10">
              ...
              <h2 className="text-2xl font-bold text-center text-slate-900 dark:text-slate-100 mb-1">
                {lecturer.fullName}
              </h2>
              <p className="text-slate-500 dark:text-slate-400 font-medium">
                {lecturer.degree?.name || 'Giảng viên'}
              </p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              <div className="bg-green-50 dark:bg-green-900/20 rounded-2xl p-4 text-center border border-green-100 dark:border-green-800">
                <span className="block text-3xl font-bold text-green-600 dark:text-green-400 mb-1">
                  {lecturer.upvoteCount}
                </span>
              ...
              <div className="bg-red-50 dark:bg-red-900/20 rounded-2xl p-4 text-center border border-red-100 dark:border-red-800">
                <span className="block text-3xl font-bold text-red-500 dark:text-red-400 mb-1">
                  {lecturer.downvoteCount}
                </span>
            ...
            <div className="space-y-6 flex-1">
              ...
              <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-700/30 rounded-xl">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xl font-bold text-slate-900 dark:text-slate-100">
                    {lecturer.teachingAssignments.reduce(
                      (acc: number, curr: TeachingAssignment) => acc + (curr.reviewsCount || 0),
                      0,
                    )}
                  </span>
