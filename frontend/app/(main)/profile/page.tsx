      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl p-8 shadow-sm border border-slate-100 dark:border-slate-700 transition-colors duration-300">
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
              ...
              <>
                ...
                <div className="pt-2 flex gap-2 justify-center md:justify-start flex-wrap">
                  {user.roles.map((r) => (
                    <span
                      key={r}
                      className="px-3 py-1 bg-slate-100 dark:bg-slate-700 rounded-full text-xs font-medium uppercase text-slate-600 dark:text-slate-300"
                    >
                      {r}
                    </span>
                  ))}
          ...
          <div className="text-center bg-blue-50 dark:bg-blue-900/30 px-6 py-4 rounded-xl">
            ...
            <div className="text-xs text-blue-800 dark:text-blue-300 font-medium uppercase tracking-wide mt-1">
              Nhận xét
            </div>
          </div>
        </div>
      ...
      <div>
        ...
        {isReviewsLoading ? (
          <div className="text-center py-12">
            <Loader2 className="w-8 h-8 animate-spin mx-auto text-slate-300 dark:text-slate-600" />
          </div>
        ) : reviews.length > 0 ? (
          <div className="grid gap-4">
            {reviews.map((review) => (
              <div
                key={review.id}
                className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow group"
              >
