...
export const bulkImportLecturers = async (
  ...
  logger.success(`Bulk import lecturers: ${results.success}/${results.total} thành công`);
 ...
 */
export const bulkImportTerms = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
...
export const bulkImportAssignments = async (
  ...
  for (let i = 0; i < assignments.length; i++) {
    ...
    const lecturerStaffId =
      assignment.lecturerStaffId ||
      assignment.lecturer_staff_id ||
      assignment.staff_id ||
      assignment.lecturer_id;
    const subjectCode = assignment.subjectCode || assignment.subject_code || assignment.subject_id;
    const termCode =
      assignment.termCode || assignment.term_code || assignment.term_name || assignment.term_id;
    ...
    try {
      ...
      if (!lecturerStaffId || !subjectCode || !termCode) {
        results.errors.push({
          ...
          data: assignment,
          error:
            'Missing required fields: lecturerStaffId (or staff_id), subjectCode (or code), termCode (or term_code)',
        });
        continue;
      }
  ...
  logger.success(`Bulk import assignments: ${results.success}/${results.total} thành công`);
